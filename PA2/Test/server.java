...
//		return Arrays.equals(rcvHash, checkHash);
	...
	private ArrayList<Packet> retrieveFile(Packet rcvPacket, Connection connection) { //seq & ack numbers & rcvWind are not set and should be before they are sent
		...
		Packet endOfFilePacket = new Packet(rcvPacket.getSessionID(), 0, 0, (byte) 0, (byte) 0, (byte) 0, (byte) 0, (byte) 0, connection.getRcvWind(), new byte[0]);
		
		byte[] fnameArray = rcvPacket.getData();
		String filename = new String(fnameArray);
		File fnameFile = new File("test.pdf");//filename);
		...
		if (Files.exists(fnameFile.toPath(), LinkOption.NOFOLLOW_LINKS)) {
			...
			int intFileIndex = 0;
			int maxDataPerPacket = Packet.MAXPACKETSIZE - 37;
			...
			for (int i = 0; i < numPackets; i++) {
				...
				Packet tempPacket = new Packet(rcvPacket.getSessionID(), 0, 0, (byte) 0, (byte) 1, (byte) 0, (byte) 0, (byte) 1, connection.getRcvWind(), temp);
